<script context="module">
  ...
  export async function preload(_, session) {
    const history = await axios.get(server + "records", makeAuth(session));
...
<script>
  ...
  async function saveRecord(rec) {
    const res = await axios.post(server + "records", rec, makeAuth($session));
    console.log(res);
  }
  ...
  async function deleteRecord({ id }) {
    await axios.delete(server + "records/" + id, makeAuth($session));
    history.splice(
...
<section>
  <div class="search">
    ...
    <button id="search" on:click={() => loadResults()}>
      <svg xmlns:svg="http://www.w3.org/2000/svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500.00001 500.00001">
        <defs id="defs4164" />
  ...
  <div class="history">
    {#each history as hist}
      <div class="record" on:click={() => load(hist)} class:current={res && hist.youtubeId === res.youtubeId}>
